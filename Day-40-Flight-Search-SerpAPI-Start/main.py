...
from flight_data import modified_find_cheapest_flight
...
# Taking note of the verified name of the email column in the sheet.
customer_email_list = [row["What is your email?"] for row in customer_data]
...
for destination in sheet_data:
    ...
    flights = flight_search.modified_check_flights(
        ORIGIN_CITY_IATA,
        destination["iataCode"],
        from_time=tomorrow,
        to_time=six_month_from_today
    )
    cheapest_flight = modified_find_cheapest_flight(flights, return_date=six_month_from_today.strftime("%Y-%m-%d"))
    ...
    if cheapest_flight.price == "N/A":
        ...
        stopover_flights = flight_search.modified_check_flights(
            ORIGIN_CITY_IATA,
            destination["iataCode"],
            from_time=tomorrow,
            to_time=six_month_from_today,
        ...
        cheapest_flight = modified_find_cheapest_flight(stopover_flights, return_date=six_month_from_today.strftime("%Y-%m-%d"))
        ...
        #Send email out to customers
        notification_manager.send_emails(
            customers_email=customer_email_list,
            email_body=f"Low price alert! Only GBP {cheapest_flight.price} to fly "
                       f"from {cheapest_flight.origin_airport} to {cheapest_flight.destination_airport},"
                       f"on {cheapest_flight.out_date} until {cheapest_flight.return_date}."
        )